 ...
 *    弹层内可管理快捷语（分类/标题/内容增删改）并从 MD 文档导入。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 解析 MD 文本 -> [{ name, prompts: [{ title, text }] }]
     * 规则：`### xxx` = 分类；顶层无序列表项 = 标题；向内缩进的列表项 = 内容（多条拼接为多行）。
     */
    ...
    /** 把导入的分类合并进现有分类：同名分类合并、同名标题跳过，返回新数组。 */
    function mergeCategories(existing, imported) {
      var result = (existing || []).map(function (c) {
        return { id: c.id, name: c.name, prompts: (c.prompts || []).map(function (p) { return { id: p.id, title: p.title, text: p.text } }) }
      })
      ;(imported || []).forEach(function (ic) {
        ...
        ic.prompts.forEach(function (p) {
          ...
          if (!dup) target.prompts.push({ id: genId('p'), title: p.title, text: p.text })
        })
      })
        ...
        React.createElement('div', { className: 'qp-titles' },
          ...
          !hasCats
            ...
            : (cur.prompts || []).length === 0
              ...
              : cur.prompts.map(function (p, j) {
                return React.createElement('button', {
                  type: 'button',
                  key: p.id || j,
                  className: 'qp-tchip',
                  title: p.text || p.title || '',
                  onClick: function () { insertIntoInput(inputActions, p.text) },
                }, p.title || '(无标题)')
              })
        )
      )
    ...
    function QuickPromptSeat(props) {
      ...
      function insertAndClose(p) {
        if (insertIntoInput(inputActions, p.text)) close()
      }
      ...
      function startEdit() {
        ...
        ;(cats || []).forEach(function (c) {
          ;(c.prompts || []).forEach(function (p) {
            rows.push({ id: p.id || '', cat: c.name || '', title: p.title || '', text: p.text || '' })
          })
        })
        setDraft(rows)
        setError(null)
      ...
      function newRow() { setDraft(draft.concat([{ id: '', cat: '', title: '', text: '' }])) }
      ...
      function regroup(rows) {
        ...
        rows.forEach(function (r) {
          ...
          if (title || text) c.prompts.push({ id: r.id || genId('p'), title: title, text: text })
        })
        ...
        file.text().then(function (text) {
          ...
          if (editing) {
            ...
            merged.forEach(function (c) {
              c.prompts.forEach(function (p) { rows.push({ id: p.id || '', cat: c.name, title: p.title, text: p.text }) })
            })
            setDraft(rows)
                    ...
                    : cats.map(function (c, ci) {
                      return React.createElement(React.Fragment, { key: c.id || ci },
                        ...
                        (c.prompts || []).length === 0
                          ...
                          : c.prompts.map(function (p, i) {
                            return React.createElement('button', { type: 'button', className: 'qp-item', key: p.id || i, title: p.text, onClick: function () { insertAndClose(p) } },
