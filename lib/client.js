...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '.qp-edit-order{font-size:13px;padding:6px 8px;border:1px solid var(--dsw-alias-border-l2-darkmode-thin,#ccc);border-radius:6px;background:var(--dsw-specific-input-minor,#fff);color:inherit;width:58px;flex:none}',
                ...
                draft.map(function (p, i) {
                  return React.createElement('div', { className: 'qp-edit-row', key: i },
                    React.createElement('div', { className: 'qp-edit-meta' },
                      ...
                      React.createElement('input', { className: 'qp-edit-order', type: 'number', min: 1, step: 1, placeholder: '序', title: '序号：同分类内按序号从小到大、自左向右排列（新增时自动给出下一个序号）', value: p.order, onChange: function (e) { updateRow(i, 'order', e.target.value) } }),
