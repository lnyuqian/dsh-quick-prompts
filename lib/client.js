...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 把导入的分类合并进现有分类：同名分类合并、同名标题跳过，返回新数组（保留 autoSend）。 */
    function mergeCategories(existing, imported) {
      var result = (existing || []).map(function (c) {
        return { id: c.id, name: c.name, prompts: (c.prompts || []).map(function (p) { return { id: p.id, title: p.title, text: p.text, autoSend: p.autoSend === true } }) }
      })
      ;(imported || []).forEach(function (ic) {
        ...
        ic.prompts.forEach(function (p) {
          ...
          if (!dup) target.prompts.push({ id: genId('p'), title: p.title, text: p.text, autoSend: p.autoSend === true })
        })
      })
    ...
    function QuickPromptSeat(props) {
      ...
      function startEdit() {
        ...
        ;(cats || []).forEach(function (c) {
          ;(c.prompts || []).forEach(function (p) {
            rows.push({ id: p.id || '', cat: c.name || '', title: p.title || '', text: p.text || '', autoSend: p.autoSend === true })
          })
        })
        setDraft(rows)
        setError(null)
      ...
      function newRow() { setDraft(draft.concat([{ id: '', cat: '', title: '', text: '', autoSend: false }])) }
      ...
      function regroup(rows) {
        ...
        rows.forEach(function (r) {
          ...
          if (title || text) c.prompts.push({ id: r.id || genId('p'), title: title, text: text, autoSend: r.autoSend === true })
        })
                      ...
                      return React.createElement(React.Fragment, { key: c.id || ci },
                        ...
                        (c.prompts || []).length === 0
                          ...
                          : c.prompts.map(function (p, i) {
                            return React.createElement('button', { type: 'button', className: 'qp-item', key: p.id || i, title: (p.autoSend ? '自动发送：' : '') + (p.text || ''), onClick: function () { insertAndClose(p) } },
                              React.createElement('span', { className: 'qp-item-title' },
                                React.createElement('span', { className: 'qp-title-wrap' },
                                  p.title || '(无标题)',
                                  p.autoSend ? React.createElement(EnterIcon) : null)),
