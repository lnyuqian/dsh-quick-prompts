 ...
 * 数据格式 v2：{ version: 2, categories: [{ id, name, prompts: [{ id, title, text, autoSend, order }] }] }
 * autoSend 为可选布尔（缺省 false = 仅插入不发送）；order 为可选数字序号（展示按其升序）；
 * 旧数据缺该字段时 autoSend 按 false、order 视为无序号处理。
  ...
  try {
    ...
    const prompts = (Array.isArray(list) ? list : [])
      ...
      .map((p) => {
        ...
        const order = sanitizeOrder(p.order)
        if (order !== undefined) np.order = order
        return np
      })
...
/** 规范化单条快捷语的可选数字序号：非法输入丢弃，合法输入取 1 起的整数。 */
