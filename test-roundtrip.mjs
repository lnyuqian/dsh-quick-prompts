...
const genId = (p) => p + 'x'
const { parseQuickPromptsMd: parse } = new Function('genId', extract('parseQuickPromptsMd') + '; return { parseQuickPromptsMd }')(genId)

// 与 client.js exportMd 相同的序列化逻辑
...
assert(back.length === 2, '分类数还原')
assert(back[0].prompts[0].text === '请翻译：\n保持语气自然。', '多行内容往返一致')
assert(back[0].prompts[1].title === '无内容条目' && back[0].prompts[1].text === '', '空内容条目往返一致')
